...
def find_all_templates(agent_id=None, documents_dir=None):
    ...
    查找逻辑：
    1. 企业内部文件 agent_{id}/手册/ → 递归搜索所有 .docx/.doc/.xlsx 文件
...
def _validate_docx(path):
    """确认转换产物是包含 Word 主文档的有效 OOXML 文件。"""
    candidate = Path(path)
...
def _convert_with_libreoffice(
    ...
    """用独立 profile 和明确过滤器执行一次 LibreOffice 转换。"""
...
def _convert_via_rtf(soffice_path, source_path, output_dir):
    """直接 DOCX 失败时，用 RTF 中间格式修复旧 DOC 的可读结构。"""
...
def convert_doc_to_docx(doc_path):
    ...
    try:
        ...
        # 旧版 Word 过滤器在 Windows 上可能被中文/特殊字符路径卡住；转换时使用
        # 本次请求独享的纯 ASCII 文件名，最终输出名仍由 routes.py 使用原文件名生成。
...
def _iter_existing_xlsx_cells(worksheet):
    """遍历文件中真实存在的单元格，避免被整列格式撑大的空白区域拖垮。"""
...
def build_xlsx_overview_chunks(
    ...
    """把所有非空、非公式单元格分块，供 LLM 逐块检查。

    不按工作簿总大小截断，避免后半部分工作表漏检；这里只分块控制单次提示词长度。
    """
    entries = []
...
def build_xlsx_llm_prompt(
    ...
    """构造 XLSX 修改提示词，响应格式为逐行 JSON（NDJSON）。"""
    system = (
        "你是一名 IATF 16949/ISO 9001 体系文件专家。你需要结合企业体系调研信息，"
        "逐个检查 Excel 模板中本批次列出的每一个单元格，并只对确实需要企业化的内容提出修改。\n\n"
    ...
    if isinstance(old_value, datetime):
        for candidate in (text, text.replace('/', '-')):
            try:
                ...
                if old_value.time() == datetime.min.time() and parsed.time() == datetime.min.time():
                    return parsed
                return parsed
            except ValueError:
                continue
...
def apply_xlsx_modifications(workbook, modifications):
    """按“全局替换优先、精确单元格覆盖随后”的顺序应用 XLSX 修改。"""
    stats = {
        'xlsx_cell': 0,
        'xlsx_global_replace': 0,
        'unknown': 0,
        'failed': 0,
    }
    globals_seen = set()
    ...
    for modification in cell_modifications:
        try:
            if apply_xlsx_cell_replace(
                workbook,
                modification.get('sheet'),
                modification.get('cell'),
